...
function convFahrenheit(tempC) {
    const tempF = (tempC * (9/5) + 32);
    return(`Grados Fahrenheit: ${tempF.toFixed(1)}ºF.`);
}

function convKelvin(){
    const tempK = tempC + 273.15;
    return(`Grados Kelvin: ${tempK.toFixed(2)}ºK.`);
}
let tempC;
do {
    try{
        tempC = parseFloat(prompt("ingrese la temperatura en grados Celsius"));
        if(isNaN(tempC)){
            console.log("Error: El dato ingresado no es un número, intente nuevamente")
        }
    }catch(error){
...
const resultadoF = convFahrenheit(tempC);
const resultadoK = convKelvin(tempC);
